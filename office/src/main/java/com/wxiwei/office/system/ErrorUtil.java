    ...
    public void writerLog(Throwable ex, boolean isReaderFile, boolean isShowErrorDialog)
    {
        try
        {
            ...
            for (int i = 0; i < ex.getStackTrace().length; i++) {
                stringBuilder.append(ex.getStackTrace()[i].toString()).append("\n");
            }
            sysKit.getControl().getMainFrame().newFatalOccurs(stringBuilder.toString());

            // If outOfMemoryError, prompt user to end application running
            if (ex instanceof OutOfMemoryError) {
                sysKit.getControl().getMainFrame().outOfMemoryOccurs();
                return;
            }
            ...
            control.getActivity().getWindow().getDecorView().post(new Runnable()
            {
                ...
                {                 
                    ...
                    {                 
                        ...
                        {
                            err = control.getActivity().getString(R.string.dialog_insufficient_memory);
                        ...
                        {
                            err = control.getActivity().getString(R.string.dialog_format_error);
                            errorCode = BAD_FILE;
                        }
                        else if (error.contains("The document is really a RTF file"))
                        {
                            err = control.getActivity().getString(R.string.dialog_rtf_file);
                            errorCode = RTF_DOCUMENT;
                        }
                        else if (ex instanceof OldFileFormatException )
                        {
                            err = control.getActivity().getString(R.string.dialog_old_document);
                            errorCode = OLD_DOCUMENT;
                        }
                        else if (error.contains("Cannot process encrypted office file"))
                        {
                            err = control.getActivity().getString(R.string.dialog_cannot_encrypted_file);
